<template>
  <form @submit.prevent="submitForm">
    ...
    <div style="border: 1px solid var(--color1)">
      <div style="overflow-y: scroll; height: 5em">
        <div v-for="service in services" @click="selectService(service)">
          <input type="checkbox" v-bind:checked=selected_services.includes(service)>
    ...
    <select v-model="selected_worker">
      <option v-for="worker in workers" :value=worker>{{ worker.name }}</option>
    </select>
    ...
    <div v-if="show_force" style="border: 1px; background: tomato">
      <p class="bold">Время занято, все равно сохранить?</p>
      <input
        type="checkbox"
        v-model="force"
      />
...
<script lang="ts">
import { DefaultService, TimeSlotType, type OutService, type OutWorker } from "@/client";
  ...
  methods: {
    selectService(service: OutService) {
      if (this.selected_services.includes(service)){
        this.selected_services = this.selected_services.filter((s) => s != service)
      } else {
    ...
    async submitForm() {
      ...
      const date_to = date_from.add(this.length_hours, 'h').add(this.length_minutes, 'm')
      try {
        await DefaultService.createSlotWithCheck({
            ...
            customer_info: {},
            services: this.selected_services.map((x) => {return {service_id: x.service_id}}),
