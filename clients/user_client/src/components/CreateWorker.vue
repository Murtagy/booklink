<template>
  <div>
    ...
    <input v-model.trim="job_title" placeholder="Должность/Роль (например: Визажист)">
...
export default {
  ...
  methods: {
    async createWorker() {
      if (!this.name) { alert('Поле имя не может быть пустым'); return }
      if (!this.job_title) { alert('Поле должность не может быть пустым'); return  }
      const worker = (await DefaultService.createWorker({name: this.name, job_title: this.job_title}));
