  ...
  <div class="box">
    <div v-if="!submitted">
      <form>
        <div id="inputs">
          <p class="bold">
            <label for="phone">Номер телефона (мобильный): </label>
          </p>
          <input
            id="phone"
            name="phone"
          ...
          <p class="bold">
            <label for="remind_me"> Напомнить о записи? </label>
          </p>
      ...
      <div class="cards">
        <div class="card">
          <div class="border_main1" style="padding: 1em">
            <div v-if="worker">{{ worker.name }}, {{ worker.job_title }}</div>
            <div v-else>Не важно</div>
          </div>
        </div>
      </div>
    ...
    <div v-else>
      ...
      <div v-else>
        <div v-if="!booked_visit">Загрузка...</div>
        <div v-else>
...
import type { OutVisitExtended } from "@/client/models/OutVisitExtended";

declare interface ComponentData {
  booked_visit: null | OutVisitExtended;
  error: string;
  first_name: string;
  last_name: string;
  phone: string;
  ...
  methods: {
    ...
    async submit_visit() {
      ...
      try {
        ...
        console.log(error);
        this.error =
          "Увы, что-то пошло не так, возможно время уже не доступно. Попробуйте записаться еще раз.";
      }
    },
  },
  props: {
